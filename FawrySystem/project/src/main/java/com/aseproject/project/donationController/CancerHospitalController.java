...
import com.aseproject.project.donationService.CancerHospital;
...
@RestController
public class CancerHospitalController {
    @Autowired
    private CancerHospital cancerHospital;
    @Autowired
    ...
    //Get request to donate to cancer hospital and pay by visa (http://localhost:8082/cancerHospitalDonation/visa/{cardNumber}/{password})
    @PostMapping("/cancerHospitalDonation/visa/{cardNumber}/{password}")
    public String Recharge(@RequestBody DonationAccount donationAccount, @PathVariable("cardNumber") String cardNumber, @PathVariable("password") int password){
        ...
        paymentTransactionService.addPaymentTransaction(new PaymentTransaction(donationAccount.getAccount().getEmail(), "CancerHospital Donation", amount, fees));
        return cancerHospital.donate(donationAccount.getDonation(), userService.getUserById(accountService.getId()).getVisa());
    }


    //Get request to donate to cancer hospital and pay by wallet (http://localhost:8082/cancerHospitalDonation/wallet)
    @PostMapping("/cancerHospitalDonation/wallet")
    public String Recharge(@RequestBody DonationAccount donationAccount){
        ...
        paymentTransactionService.addPaymentTransaction(new PaymentTransaction(donationAccount.getAccount().getEmail(), "CancerHospital Donation", amount, fees));
        return cancerHospital.donate(donationAccount.getDonation(), userService.getUserById(accountService.getId()).getWallet());
    }
}
