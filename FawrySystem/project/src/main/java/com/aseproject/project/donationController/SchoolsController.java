...
import com.aseproject.project.donationService.Schools;
...
public class SchoolsController {
    ...
    //Get request to donate to schools and pay by visa (http://localhost:8082/schoolDonation/visa/cardNumber/password)
    @PostMapping("/schoolDonation/visa/{cardNumber}/{password}")
    public String Recharge(@RequestBody DonationAccount donationAccount, @PathVariable("cardNumber") String cardNumber, @PathVariable("password") int password){
        ...
        paymentTransactionService.addPaymentTransaction(new PaymentTransaction(donationAccount.getAccount().getEmail(), "Schools Donation", amount, fees));
        return schools.donate(donationAccount.getDonation(), userService.getUserById(accountService.getId()).getVisa());
    }

    //Get request to donate to schools and pay by wallet (http://localhost:8082/schoolDonation/wallet)
    @PostMapping("/schoolDonation/wallet")
    public String Recharge(@RequestBody DonationAccount donationAccount){
        ...
        paymentTransactionService.addPaymentTransaction(new PaymentTransaction(donationAccount.getAccount().getEmail(), "Schools Donation", amount, fees));
        return schools.donate(donationAccount.getDonation(), userService.getUserById(accountService.getId()).getWallet());
    }
}
